...
|---|---|---|---|---|
| **Set Calculus** | A provenance-preserving transformation and resolution framework built around Set, Member, Relationship, Identity, State, Transform, Resolution, and Provenance. | Core formal system | SC | docs/set-calculus-core/README.md; SCOPE.md |
| **Set** | A formally identified collection or resolution domain whose admitted Members and Relationships are evaluated under explicit Context, Authority, and Provenance. | Set Calculus primitive | set only when the Core primitive is intended | docs/set-calculus-core/README.md |
| **Member** | An identified object admitted to a Set. Membership states participation in the Set and does not collapse the Member's Identity into its current State. | Set Calculus primitive | element when used as a mathematical alias | docs/set-calculus-core/README.md; docs/set-calculus-core/TRAJECTORY_ADMISSIBILITY_AND_RESOLUTION_DEPTH.md |
| **Relationship** | A typed, scoped relation connecting identified objects, Sets, or States, with materially required evidence and Provenance under the active Context and Authority. | Set Calculus primitive | relation when no structural distinction is lost | docs/set-calculus-core/README.md; docs/set-calculus-core/TRAJECTORY_ADMISSIBILITY_AND_RESOLUTION_DEPTH.md |
| **Identity** | The provenance-bearing criterion that determines what object is being tracked across States, Relationships, boundaries, and Transforms. Identity is evaluated independently of State. | Set Calculus primitive | identity reference when referring to its identifier | docs/set-calculus-core/README.md; docs/set-calculus-core/TRAJECTORY_ADMISSIBILITY_AND_RESOLUTION_DEPTH.md |
...
| **Resolution Transition** | A provenance-bearing recomputation from one public resolution state to another under an explicit material cause, with the destination state determined by the active evidence state and rule profile. | Set Calculus formal transition | reclassification when the public state changes | docs/set-calculus-core/RESOLUTION_TRANSITION_SEMANTICS.md |
| **Closure/Reopening Algebra** | The partial algebra over closure-layer configurations with CLOSE, EXTEND, REOPEN, RECLOSE, identity, typed composition, epoch order, active-receipt uniqueness, and append-only closure history. | Set Calculus formal algebra | closure algebra; A10 | docs/set-calculus-core/CLOSURE_REOPENING_ALGEBRA.md |
